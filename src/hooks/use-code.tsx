...
const useCode = () => {
  ...
  const parseCode = useCallback(
    (dataCode) => {
      ...
      const isTotalInsurer =
        filters.insurer.name &&
  ...
  const handleGetCode = (dataCode) => async () => {
    ...
    await axios.post(baseURLApi + '/data', { code: query }).then((response) => {
      const data = response.data;
      setValue(data?.value);
    });
  };

  const handleBatchCode = useCallback(
    async (arrayCodes) => {
      ...
      try {
        const response = await axios.post(baseURLApi + '/data/batch', {
          codeList: [...batch],
        });
        const data = response.data;
