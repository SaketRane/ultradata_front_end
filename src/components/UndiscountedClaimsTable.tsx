...
const UndiscountedClaimsTable: React.FC = () => {
  // Function to generate data cell code based on row code and column type
          ...
          {tableRows.map((row, index) => {
            ...
            return (
              <TableRow key={index} className={bgClass} data-row-code={row.rowCode}>
                ...
                {/* Direct claims column */}
                <TableCell
                  className={`text-center py-1 px-2 ${sizeClass} ${fontClass}`}
                  data-code={generateDataCode(row.rowCode, columnCodes.direct)}
                >
                  {row.rowCode && generateDataCode(row.rowCode, columnCodes.direct) && (
                    <span className="text-green-600 opacity-0 hover:opacity-50 text-[7px]">
                      {generateDataCode(row.rowCode, columnCodes.direct)}
                    </span>
                  )}
                </TableCell>
                
                {/* Reinsurance assumed column */}
                <TableCell
                  className={`text-center py-1 px-2 ${sizeClass} ${fontClass}`}
                  data-code={generateDataCode(row.rowCode, columnCodes.reinsuranceAssumed)}
                >
                  {row.rowCode && generateDataCode(row.rowCode, columnCodes.reinsuranceAssumed) && (
                    <span className="text-green-600 opacity-0 hover:opacity-50 text-[7px]">
                      {generateDataCode(row.rowCode, columnCodes.reinsuranceAssumed)}
                    </span>
                  )}
                </TableCell>
                
                {/* Reinsurance ceded column */}
                <TableCell
                  className={`text-center py-1 px-2 ${sizeClass} ${fontClass}`}
                  data-code={generateDataCode(row.rowCode, columnCodes.reinsuranceCeded)}
                >
                  {row.rowCode && generateDataCode(row.rowCode, columnCodes.reinsuranceCeded) && (
                    <span className="text-green-600 opacity-0 hover:opacity-50 text-[7px]">
                      {generateDataCode(row.rowCode, columnCodes.reinsuranceCeded)}
                    </span>
                  )}
                </TableCell>
                ...
                <TableCell
                  className={`text-center py-1 px-2 ${sizeClass} ${fontClass}`}
                  data-code={generateDataCode(row.rowCode, columnCodes.netIncurred)}
                >
                  {row.rowCode && generateDataCode(row.rowCode, columnCodes.netIncurred) && (
                    <span className="text-green-600 opacity-0 hover:opacity-50 text-[7px]">
                      {generateDataCode(row.rowCode, columnCodes.netIncurred)}
                    </span>
                  )}
                </TableCell>
              </TableRow>
...
export default UndiscountedClaimsTable;
